...
var SpaceShip = function () {
  ...
  local.board.build.fieldGrid = function () {
    ...
    // set the grid
    local.objects.board.style.gridTemplateColumns = (
      local.settings.spaceSize + "px "
    ).repeat(local.settings.columns);
    local.objects.board.style.gridTemplateRows = (
      local.settings.spaceSize + "px "
    ).repeat(local.settings.rows);
    ...
    for (var i = 0; i < local.settings.rows * local.settings.columns; i++) {
      ...
      local.objects.board.appendChild(
        local.objects.spaces[coordinates[0]][coordinates[1]]
      );
    }
  };
  local.board.build.fieldObjects = function () {
    if (local.settings.finish != false) {
      local.space.addClass(
        local.settings.finish[0],
        local.settings.finish[1],
        "finish"
      );
    }
    ...
    for (var i = 0; i < local.settings.pits.length; i++) {
      local.space.addClass(
        local.settings.pits[i][0],
        local.settings.pits[i][1],
        "pit"
      );
    }
    ...
    for (var i = 0; i < local.settings.voids.length; i++) {
      local.space.addClass(
        local.settings.voids[i][0],
        local.settings.voids[i][1],
        "void"
      );
    }
    ...
    for (var i = 0; i < local.settings.colored.length; i++) {
      local.space.addClass(
        local.settings.colored[i][0],
        local.settings.colored[i][1],
        "colored"
      );
    }
    ...
    for (var i = 0; i < local.settings.walls.length; i++) {
      for (var j = 0; j < local.settings.walls[i][2].length; j++) {
        local.space.addClass(
          local.settings.walls[i][0],
          local.settings.walls[i][1],
          "wall-" + local.settings.walls[i][2][j]
        );
      }
    }
  ...
  local.board.build.actionBar = function () {
    ...
    actionBar.style.height =
      local.settings.rows * local.settings.spaceSize + "px";
  ...
  local.board.writeToActionBar = function (actionName, success) {
    ...
    setTimeout(function () {
      action.style.height =
        (local.settings.rows * local.settings.spaceSize) /
          local.state.orignalProgrammerActionCount -
        2 +
        "px";
    }, 30);
  };
  ...
  local.ship.isOnFinish = function () {
    return local.space.containsShip(
      local.settings.finish[0],
      local.settings.finish[1]
    );
  };
  local.ship.isOnBelt = function () {
    return local.objects.spaces[local.state.ship.position[0]][
      local.state.ship.position[1]
    ].classList.contains("belt");
  };
  local.ship.isOnPit = function () {
    return local.objects.spaces[local.state.ship.position[0]][
      local.state.ship.position[1]
    ].classList.contains("pit");
  };
  local.ship.move = {};
  ...
  local.ship.move.go = function () {
    ...
    if (move) {
      ...
      var currentSpace =
        local.objects.spaces[currentPosition[0]][currentPosition[1]];
      if (
        currentSpace.classList.contains("wall-" + wallCheck[0]) ||
        newSpace.classList.contains("wall-" + wallCheck[1])
      ) {
  ...
  local.ship.rotate.transform = function (toRight) {
    ...
    local.objects.ship.style.transform =
      "rotate(" + local.state.ship.deg + "deg)";
    local.objects.ship.style.webkitTransform =
      "rotate(" + local.state.ship.deg + "deg)";
  };
  local.ship.setOrientation = function (toRight) {
    var index =
      local.orientations.indexOf(local.state.ship.orientation) +
      (toRight ? -1 : 1);
  ...
  local.ship.destroy = function () {
    local.space
      .getElement(local.state.ship.position[0], local.state.ship.position[1])
      .removeChild(local.objects.ship);
  ...
  local.ship.checkState = function () {
    ...
    try {
      if (local.ship.isOnBelt()) {
        ...
        setTimeout(function () {
          var orientation = local.state.ship.orientation;
          local.state.ship.orientation = local.space.getData(
            local.state.ship.position[0],
            local.state.ship.position[1],
            "direction"
          );
          local.ship.move.go();
        ...
        if (
          local.state.programmerActionCount > 0 &&
          local.goals.noExtraActions
        ) {
          throw "To many moves!";
        }
        ...
        local.settings.finish != false &&
        currentState.programmerActionCount == 0 &&
        ...
        local.goals.colorAll != undefined &&
        local.goals.colorAll == true &&
        local.board.isColored()
      ) {
        local.board.createBanner("Success!", "success");
      }
    } catch (e) {
  ...
  local.actions.plan = function (action, addBoardAction) {
    if (
      local.goals.maxActions != undefined &&
      local.state.programmerActionCount > local.goals.maxActions
    ) {
      local.run();
      local.board.createBanner(
        "Failure!",
        "fail",
        "Te veel acties (" +
          local.state.programmerActionCount +
          " geprogrmeerd, " +
          local.goals.maxActions +
          " toegestaan)"
      );
      throw "To many moves";
      console.clear();
  ...
  local.actions.planProgrammerAction = function (name, action, addBoardAction) {
    ...
    local.actions.plan(function () {
      if (
        local.goals.allowedFunctions != undefined &&
        local.goals.allowedFunctions.indexOf(name) == -1
      ) {
        local.board.createBanner(
          "Failure!",
          "fail",
          name + " is niet een toegestane functie"
        );
        ...
        if (local.goals.noFailedActions == true && !success) {
          local.board.createBanner(
            "Failure!",
            "fail",
            "Er heeft een gefaalde actie plaatsgevonden"
          );
  ...
  self.run = function (func) {
    ...
    if (
      local.goals.maxLines != undefined &&
      lines.length > local.goals.maxLines
    ) {
      local.board.createBanner(
        "Failure!",
        "fail",
        "Te veel regels (" +
          local.goals.maxLines +
          " toegestaan, " +
          lines.length +
          " gebruikt)"
      );
  ...
  self.rotateLeft = function () {
    local.actions.planProgrammerAction(
      "rotateLeft",
      local.ship.rotate.left,
      true
    );
  };
